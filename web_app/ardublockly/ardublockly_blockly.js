...
/*
// Original Ardublockly.discardAllBlocks
...
Ardublockly.discardAllBlocks = function() {
  ...
  if (blockCount > 1) {
    Ardublockly.alertMessage(
        ...
        true,
        function() {
          // Ardublockly.workspace.clear();  // 如果初始要空畫面，就使用這個。
          // Ardublockly.renderContent();    // 如果初始要空畫面，就使用這個。
          // 載入預設積木
          // 立刻 loadServerXmlFile 的話，有時候會直接顯示英文語系，所以進行延遲載入
          setTimeout(function () {
...
Ardublockly.updateToolboxLanguage = function() {
  var categories = ['catProcedure', 'catLogic', 'catLoops', 'catMath', 'catText',
