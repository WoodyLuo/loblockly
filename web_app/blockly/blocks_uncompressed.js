...
Blockly.Blocks.controls_switch = {
    init: function() {
        ...
        this.setColour(210);
        this.setTooltip("The expression is evaluated once and compared with the values of each case label. If there is a case matching, the corresponding code after the matching label is executed.");
        this.setHelpUrl("");
        // Mutator 是 齒輪圖示 是用來增生(擴充)原有的方塊(controls_switch)。
        this.setMutator(  new Blockly.Mutator(["controls_switch_case", "controls_switch_default"]) );
        var a = this;
        this.defaultCount_ = this.caseCount_ = 0;
    },
    // 如果有設置方塊的增生(擴充)視窗，
    // Blockly.js會做增生視窗的工作區改變偵測 (call Blockly.Mutator.workspaceChanged_())，
    // 增生視窗的改變，程式碼區會會需要立即跟著產生更新
    // 這時就會需要mutationToDom與domToMutation。
    // mutationToDom的功能：是將增生視窗的的改變，立即程式碼區的「程式新增」。
    // domToMutation的功能：是將增生視窗的的改變，立即程式碼區的「程式更新」。
    mutationToDom: function () {
        if (!this.caseCount_ && !this.defaultCount_) return null;  // 防呆檢查caseCount_與defaultCount_是否有給訂初始值
        var a = document.createElement("mutation");
        this.caseCount_ && a.setAttribute("case", this.caseCount_); // string: case + caseCount_. (caseCount_ change in 'compose' function )
        this.defaultCount_ && a.setAttribute("default", 1);
        return a;
    },
    domToMutation: function () {
        // dom是字串，需要將字串轉換成數字，才可對caseCount_與defaultCount_數字變數進行改變。
        this.caseCount_ = parseInt(a.getAttribute("case"), 10)  || 0
        this.defaultCount_ = parseInt(a.getAttribute("default", 10)) || 0;
        this.updateShape_(); // 程式碼視窗更新
    },
    decompose: function (a) { // decompose is Mutation Window 
        ...
        this.defaultCount_ &&
          ((a = a.newBlock("controls_switch_default")),
          a.initSvg(),
          c.connect(a.previousConnection));
        return b;
    },
    compose: function (a) {  
        // compose是指當增生視窗的改變(增、刪)，就需要取得改變方塊的變數。
        // 將變數依照改變方塊的類型，進行變數更動。
        var b = a.nextConnection.targetBlock();
        this.defaultCount_ = this.caseCount_ = 0;
        a = [null];
        ...
        this.updateShape_();
        for (b = 1; b <= this.caseCount_; b++)
        {
            Blockly.Mutator.reconnect(a[b], this, "CASE" + b);
            Blockly.Mutator.reconnect(c[b], this, "DO" + b);
        }
        Blockly.Mutator.reconnect(d, this, "DEFAULT");
    },
    saveConnections: function (a) {
        for (a = a.getInputTargetBlock("STACK"); a; ) {
          switch (a.type) {
            case "controls_switch_case":
              var b = this.getInput("CASE1"),
                c = this.getInput("DO1");
              a.valueConnection_ = b && b.connection.targetConnection;
              a.statementConnection_ = c && c.connection.targetConnection;
              x++;
              break;
            case "controls_switch_default":
              c = this.getInput("DEFAULT");
              a.statementConnection_ = c && c.connection.targetConnection;
              break;
            default:
              throw "Unknown block type.";
    ...
    updateShape_: function () {
        this.getInput("DEFAULT") &&
            (this.removeInput("DEFAULT"), this.removeInput("DEFAULTDO"));
    ...
    updateShape_: function () {
        ...
        this.defaultCount_ &&
          (this.appendStatementInput("DEFAULT").appendField(
            Blockly.Msg.CONTROLS_SWITCH_DEFAULT
          ),
          this.appendDummyInput("DEFAULTDO").appendField(
            Blockly.Msg.CONTROLS_SWITCH_DO
          ));
      },*/
};
Blockly.Blocks.controls_switch_switch = {
    init: function () {
      ...
      this.setNextStatement(!0);
      this.setTooltip( "The expression is evaluated once and compared with the values of each case label." );
      this.contextMenu = !1;
    },
};
Blockly.Blocks.controls_switch_case = {
    init: function () {
      ...
      this.setPreviousStatement(!0);
      this.setNextStatement(!0);
      this.setTooltip("If there is a case matching, the corresponding code after the matching label is executed.");
      this.contextMenu = !1;
    },
};
Blockly.Blocks.controls_switch_default = {
    init: function () {
      ...
      this.setPreviousStatement(!0);
      this.setTooltip("If there is no cases matching, the code after default: is executed.");
      this.contextMenu = !1;
    },
};
...
Blockly.Blocks.serial_print = {
  ...
  onchange: function () {
    if (this.workspace) {
      for (
        var a = this.getFieldValue("SERIAL_ID"),
          b = Blockly.mainWorkspace.getTopBlocks(),
          c = !1,
          d = 0;
        d < b.length;
        d++
...
Blockly.Blocks.spi_transfer = {
  ...
  onchange: function () {
    if (this.workspace) {
      for (
        var a = this.getFieldValue("SPI_ID"),
          b = Blockly.mainWorkspace.getTopBlocks(),
          c = !1,
          d = 0,
          e = b.length;
