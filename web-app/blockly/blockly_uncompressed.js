...
Blockly.Xml.domToBlock = function (a, b) {
  ...
  if (b.rendered) {
    ...
    for (var c = d.getDescendants(), e = c.length - 1; 0 <= e; e--)
      c[e].initSvg();
...
Blockly.BlockSvg.prototype.renderDrawRight_ = function (a, b, c, d, e, f, g) {
  for (var h, k = 0, l, p, m = 0, q; (q = f[m]); m++) {
    ...
    if (this.isCollapsed()) {
      ...
      for (var r = 0; (n = q[r]); r++)
        (l = k),
          q.thicker && (l += Blockly.BlockSvg.INLINE_PADDING_Y),
          ...
          n.type == Blockly.INPUT_VALUE &&
            ...
            (l = this.RTL
              ...
              : e.x +
                ...
                n.renderWidth -
                1),
            (p = e.y + k + Blockly.BlockSvg.INLINE_PADDING_Y + 1),
            n.connection.moveTo(l, p),
            n.connection.isConnected() && n.connection.tighten_());
        ...
        ? ((n = q[0]),
          ...
          n.align != Blockly.ALIGN_LEFT &&
            ...
            n.align == Blockly.ALIGN_RIGHT
              ? (h += r)
              : n.align == Blockly.ALIGN_CENTRE && (h += r / 2)),
          this.renderFields_(n.fieldRow, h, l),
          a.push(Blockly.BlockSvg.TAB_PATH_DOWN),
          (r = q.height - Blockly.BlockSvg.TAB_HEIGHT),
          a.push("v", r),
          this.RTL
          ...
          this.RTL
            ...
            : (b.push(
                ...
                h -
                  ...
                  Blockly.BlockSvg.DISTANCE_45_OUTSIDE +
                  "," +
                  (k + q.height - Blockly.BlockSvg.DISTANCE_45_OUTSIDE)
              ),
...
Blockly.FieldInstance.prototype.dropdownCreate = function () {
  ...
  for (
    var c = this.editDropdownData ? this.editDropdownData(c) : [], b = 0;
    b < a.length;
    b++
  )
    c[b] = [a[b], a[b]];
  ...
  function b(a, b, c) {
    ...
    Blockly.prompt(a, b, function (a) {
      a &&
        ...
        a == Blockly.Msg.RENAME_INSTANCE || a == Blockly.Msg.NEW_INSTANCE) &&
        (a = null);
      c(a);
    });
  }
...
Blockly.Types.getChildBlockType = function (a) {
  for (var b = a; b && void 0 === b.getBlockType && 0 < b.inputList.length; )
  ...
  var b = function (a) {
      ...
      do {
        ...
        for (var l = 0; l < d.length; l++)
          d[l].type == Blockly.NEXT_STATEMENT &&
            (c = d[l].targetBlock()) &&
            (d[l] === e ? (a = c) : (k = k.concat(b(c))));
      } while (a);
      return k;
    },
    c = [];
...
/** ARDUINO_PROCEDURE -- dynamic category custom */
Blockly.ArduinoProcedures = {};
Blockly.ArduinoProcedures.NAME_TYPE = "ARDUINO_PROCEDURE";
Blockly.ArduinoProcedures.allProcedures = function (a) {
  a = a.getAllBlocks();
  // console.log( "Blockly.ArduinoProcedures.allProcedure: ", a );
  for (var b = [], c = [], d = 0; d < a.length; d++)
    if (a[d].getProcedureDef) {
      var e = a[d].getProcedureDef();
      e && (e[2] ? b.push(e) : c.push(e));
    }
  c.sort(Blockly.ArduinoProcedures.procTupleComparator_);
  b.sort(Blockly.ArduinoProcedures.procTupleComparator_);
  return [c, b];
};
Blockly.ArduinoProcedures.getDefinition = function (a, b) {
...
Blockly.ArduinoProcedures.procTupleComparator_ = function (a, b) {
  return a[0].toLowerCase().localeCompare(b[0].toLowerCase());
};
Blockly.ArduinoProcedures.findLegalName = function (a, b) {
  if (b.isInFlyout) return a;
  for (; !Blockly.ArduinoProcedures.isLegalName(a, b.workspace, b); ) {
    var c = a.match(/^(.*?)(\d+)$/);
    a = c ? c[1] + (parseInt(c[2], 10) + 1) : a + "2";
  }
  return a;
};
Blockly.ArduinoProcedures.isLegalName = function (a, b, c) {
  b = b.getAllBlocks();
...
Blockly.ArduinoProcedures.rename = function (a) {
  a = a.replace(/^[\s\xa0]+|[\s\xa0]+$/g, "");
  a = Blockly.ArduinoProcedures.findLegalName(a, this.sourceBlock_);
  for (
...
Blockly.ArduinoProcedures.flyoutCategory = function (a) {
  function b(g, h) {
  ...
  a = Blockly.ArduinoProcedures.allProcedures(a);
  return c;
};
Blockly.ArduinoProcedures.getCallers = function (a, b) {
...
Blockly.ArduinoProcedures.disposeCallers = function (a, b) {
  for (var c = Blockly.ArduinoProcedures.getCallers(a, b), d = 0; d < c.length; d++)
    c[d].dispose(!0, !1);
};
Blockly.ArduinoProcedures.mutateCallers = function (a) {
  var b = Blockly.Events.recordUndo,
    c = a.getProcedureDef()[0],
    d = a.mutationToDom(!0);
  a = Blockly.ArduinoProcedures.getCallers(c, a.workspace);
  c = 0;
  for (var e; (e = a[c]); c++) {
...
/** PROCEDURE -- dynamic category custom */
Blockly.Procedures = {};
...
// 因為有這行程式，Blockly.Procedures.NAME_TYPE = "PROCEDURE";
// 所以可以設定設客製化方塊(blocks)在工作區(toolbox)
// 設置方式：Blockly.Procedures.flyoutCategory { ... }
Blockly.Procedures.flyoutCategory = function (a) {
  ...
  var c = [];
  var d = goog.dom.createDom("block");  // 需要註解掉，如果要加入arduino_functions方塊
  d.setAttribute("disabled", !0);       // 需要註解掉，如果要加入arduino_functions方塊
  // 停止加入 arduino_functions方塊
  /*
  if (Blockly.Blocks.arduino_functions) {
    var d = goog.dom.createDom("block");
    d.setAttribute("type", "arduino_functions");
...
Blockly.Flyout.prototype.show = function (a) {
  ...
  // 偵測 工具箱Xml客製化動態類別 - custom="PROCEDURE"
  ...
  // 偵測 工具箱Xml客製化動態類別 - custom="ARDUINO_PROCEDURE
  a == Blockly.Variables.NAME_TYPE
    ? (a = Blockly.Variables.flyoutCategory(this.workspace_.targetWorkspace))
    : a == Blockly.ArduinoProcedures.NAME_TYPE &&
        (a = Blockly.ArduinoProcedures.flyoutCategory(this.workspace_.targetWorkspace));
  // 除厝列印 print
  //console.log( "Blockly.Flyout.prototype.show: ", a);
...
Blockly.Flyout.prototype.offsetHorizontalRtlBlocks = function (a) {
  if (this.horizontalLayout_ && this.RTL) {
    ...
    for (
      var b = Math.max(-b.x + this.MARGIN, this.width_ / this.workspace_.scale),
        c = 0,
        d;
      (d = a[c]);
      c++
        ...
        switch (p.tagName.toUpperCase()) {
          case "CATEGORY":
            ...
            goog.isString(m)
              ? (m.match(/^#[0-9a-fA-F]{6}$/)
                  ? (f.hexColour = m)
                  : (f.hexColour = Blockly.hueToRgb(m)),
                (e = !0))
              : (f.hexColour = "");
            "true" == p.getAttribute("expanded")
              ? (f.blocks.length && c.setSelectedItem(f), f.setExpanded(!0))
              : f.setExpanded(!1);
            f = p;
            break;
          case "SEP":
            f &&
              ("CATEGORY" == f.tagName.toUpperCase()
                ? g.add(
                    new Blockly.Toolbox.TreeSeparator(d.treeSeparatorConfig_)
                  )
                : ((p = parseFloat(p.getAttribute("gap"))),
                  isNaN(p) ||
                    ((m = parseFloat(f.getAttribute("gap"))),
                    (p = isNaN(m) ? p : m + p),
                    f.setAttribute("gap", p))));
            break;
          case "BLOCK":
          case "SHADOW":
            g.blocks.push(p), (f = p);
...
Blockly.init_ = function (a) {
  ...
  // 因為Blockly creates some of the overlays directly into the HTML body (eg Toolbox div, Widget div and workspace svg at least).
  // 關閉Blockly SVG的右鍵彈跳的預覽器選單
  Blockly.bindEvent_(c, "contextmenu", null, Blockly.onContextMenu_);
  Blockly.bindEvent_(Blockly.WidgetDiv.DIV, "contextmenu", null, Blockly.onContextMenu_);
  // 嘗試將Blockly工具箱的context menu關閉，但沒效果
  // Blockly.bindEvent_(Blockly.Tooltip.DIV, "contextmenu", null, Blockly.onContextMenu_);
